...
from celery_worker.tasks import monitor_gmail
...
def status(current_user: HRUser = Depends(get_current_user)):
    connected = gmail_svc.is_authenticated(current_user)
...
def get_emails(
    ...
    if not gmail_svc.is_authenticated(current_user):
        raise HTTPException(status_code=401, detail="Gmail not connected.")

    query = db.query(Email).filter(Email.provider == "gmail")
...
def get_email(
    ...
    email = db.query(Email).filter_by(id=email_id, provider="gmail").first()
    if not email:
...
def download_all(
    ...
    atts = db.query(Attachment)\
             ...
             .filter(Email.provider == "gmail").all()
    zip_buffer = io.BytesIO()
    with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zf:
        for att in atts:
...
def start_monitor(
    ...
    if not gmail_svc.is_authenticated(current_user):
        raise HTTPException(status_code=401, detail="Gmail not connected.")
    if redis_client.get(f"monitor:gmail:{current_user.id}"):
        return {"message": "Monitor already running"}
    redis_client.set(f"monitor:gmail:{current_user.id}", "running")
    monitor_gmail.apply_async(args=[current_user.id])
    return {"message": " Gmail monitor started — checking every 10 minutes"}


@router.post("/monitor/stop", response_model=MessageResponse)
def stop_monitor(current_user: HRUser = Depends(get_current_user)):
    redis_client.delete(f"monitor:gmail:{current_user.id}")
    return {"message": " Gmail monitor stopped"}


@router.get("/monitor/status", response_model=MonitorStatus)
def monitor_status(current_user: HRUser = Depends(get_current_user)):
    is_running = redis_client.get(f"monitor:gmail:{current_user.id}") is not None
    data       = redis_client.get(f"last_run:gmail:{current_user.id}")
...
def manual_sync(
    ...
    if not gmail_svc.is_authenticated(current_user):
        raise HTTPException(status_code=401, detail="Gmail not connected.")
    count = gmail_svc.fetch_and_store_emails(current_user, db)
